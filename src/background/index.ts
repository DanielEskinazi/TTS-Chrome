...
try {
  ...
  // Initialize keyboard commands after all managers are set up
  initializeKeyboardCommands();
  
  debugLog('Background script loaded and ready');
  debugLog('Service worker started successfully');
  console.log('[Keyboard-Debug] Background script initialization complete');
} catch (error) {
  console.error('Failed to initialize background script:', error);
  debugLog('Service worker initialization failed:', error);
}

// Keyboard command handlers - will be initialized after managers are ready
async function handleStartTTSCommand() {
  try {
    console.log('[Keyboard-Debug] Start TTS command triggered');
    debugLog('[Keyboard-Debug] Start TTS command triggered');
    
    // Check if managers are initialized
    if (!ttsManager) {
      console.error('[Keyboard-Debug] TTS Manager not initialized');
      debugLog('TTS Manager not initialized for keyboard command');
      return;
    }
    
    if (!voiceManager) {
      console.error('[Keyboard-Debug] Voice Manager not initialized');
      debugLog('Voice Manager not initialized for keyboard command');
      return;
    }
    ...
    if (!activeTab || !activeTab.id) {
      console.log('[Keyboard-Debug] No active tab found');
      debugLog('No active tab found for keyboard command');
      return;
    }

    console.log('[Keyboard-Debug] Active tab found:', activeTab.url);
    ...
    if (activeTab.url && (activeTab.url.startsWith('chrome://') || 
      ...
      console.log('[Keyboard-Debug] Cannot use TTS on this page:', activeTab.url);
      debugLog('Cannot use TTS on this page:', activeTab.url);
      return;
    }

    // Get selection from the active tab
    console.log('[Keyboard-Debug] Getting selection from tab');
    const response = await chrome.tabs.sendMessage(activeTab.id, {
      type: MessageType.GET_SELECTION
    }).catch(error => {
      console.log('[Keyboard-Debug] Could not get selection from tab:', error);
      debugLog('Could not get selection from tab:', error);
      return null;
    });

    console.log('[Keyboard-Debug] Selection response:', response);

    if (!response || !response.hasSelection || !response.text) {
      console.log('[Keyboard-Debug] No text selected');
      ...
      try {
        chrome.notifications?.create({
          type: 'basic',
          iconUrl: 'icon-48.png',
          title: 'TTS Keyboard Shortcut',
          message: 'Please select some text first, then press Ctrl+Shift+S to start reading.'
        });
      } catch (notificationError) {
        console.log('[Keyboard-Debug] Could not show notification:', notificationError);
      }
      return;
    }

    console.log('[Keyboard-Debug] Starting TTS with text:', response.text.substring(0, 50) + '...');
    ...
    console.log('[Keyboard-Debug] TTS started successfully via keyboard shortcut');
    debugLog('TTS started via keyboard shortcut');
    
  } catch (error) {
    console.error('[Keyboard-Debug] Error handling start TTS keyboard command:', error);
...
async function handleStopTTSCommand() {
  try {
    console.log('[Keyboard-Debug] Stop TTS command triggered');
    debugLog('[Keyboard-Debug] Stop TTS command triggered');
    
    if (!ttsManager) {
      console.error('[Keyboard-Debug] TTS Manager not initialized');
      debugLog('TTS Manager not initialized for stop command');
      return;
    }
    
    await ttsManager.stopTTS({ source: 'keyboard' });
    console.log('[Keyboard-Debug] TTS stopped successfully via keyboard shortcut');
    debugLog('TTS stopped via keyboard shortcut');
    
  } catch (error) {
    console.error('[Keyboard-Debug] Error handling stop TTS keyboard command:', error);
    debugLog('Error handling stop TTS keyboard command:', error);
  }
}

// Function to initialize keyboard commands after managers are ready
function initializeKeyboardCommands() {
  try {
    console.log('[Keyboard-Debug] Initializing keyboard commands');
    debugLog('Initializing keyboard commands');
    
    chrome.commands.onCommand.addListener((command) => {
      console.log('[Keyboard-Debug] Keyboard command received:', command);
      ...
      switch (command) {
        ...
        case 'stop_tts':
          ...
          break;
        default:
          console.log('[Keyboard-Debug] Unknown command:', command);
          debugLog('Unknown command:', command);
      }
    });
    
    console.log('[Keyboard-Debug] Keyboard commands listener registered');
    debugLog('Keyboard commands listener registered');
    
  } catch (error) {
    console.error('[Keyboard-Debug] Error initializing keyboard commands:', error);
    debugLog('Error initializing keyboard commands:', error);
  }
}
